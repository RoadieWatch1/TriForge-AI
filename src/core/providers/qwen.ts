/**
 * Qwen Provider — uses Alibaba DashScope OpenAI-compatible API via native fetch.
 */
...
const VALIDATE_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1/models';
  ...
  private async call(messages: { role: string; content: string }[], signal?: AbortSignal): Promise<string> {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        throw new ProviderError('qwen', 0, 'Request cancelled or timed out.');
      }
      throw new ProviderError('qwen', 0, `Network error: ${err.message}`);
    }
    ...
    if (!res.ok) {
      ...
      throw new ProviderError(
        'qwen',
        res.status,
        `Qwen API error ${res.status}: ${body.substring(0, 200)}`,
    ...
    if (!content) {
      throw new ProviderError('qwen', 0, 'Qwen returned an empty response.');
    }
    return content;
  }
