    ...
    for (let round = 1; round <= this.options.maxIterations; round++) {
      ...
      for (const review of reviews) {
        if (review.verdict === 'REQUEST_CHANGES') {
          feedbackParts.push(`[${review.provider}] Issues:`);
          for (const issue of review.issues) {
            feedbackParts.push(`  - [${issue.severity}] ${issue.message}`);
          }
          for (const change of review.requiredChanges) {
            feedbackParts.push(`  Required: ${change}`);
          }
        }
      }
