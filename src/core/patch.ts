 ...
 * Prevents path traversal attacks from AI-generated patches.
 */
...
/** Active approval tokens — a token must be created via createApprovalRequest before patches can be applied. */
const activeApprovalTokens = new Set<string>();
...
export async function applyPatches(
  ...
  if (!activeApprovalTokens.has(approvalToken)) {
...
export function createApprovalRequest(patches: ChangePatch[]): ApprovalRequest {
  ...
  activeApprovalTokens.add(token);
  return {
    patches,
