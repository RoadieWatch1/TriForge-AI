...
import { AIProvider } from '../core/providers/provider';
...
suite('TriForgeOrchestrator', () => {
  ...
  test('pairReview: builder responds, reviewer adds critique', async () => {
    ...
    const reviewer = new MockProvider('qwen').setGenerateResponse('reviewer critique');
  ...
  test('orchestrate: delete action is approved immediately without review', async () => {
    ...
    const reviewer = new MockProvider('qwen');
  ...
  test('orchestrate: all reviewers approve on round 1 → file approved, 1 round', async () => {
    ...
    const reviewer = new MockProvider('qwen').setVerdicts('APPROVE');
  ...
  test('orchestrate: three providers, both reviewers approve round 1 → consensus', async () => {
    ...
    const reviewer1 = new MockProvider('qwen').setVerdicts('APPROVE');
  ...
  test('orchestrate: reviewer requests changes once, approves next round → 2 rounds', async () => {
    // 2 providers: openai builds round 1, qwen builds round 2
    // qwen reviews round 1 (REQUEST_CHANGES), openai reviews round 2 (APPROVE)
    const p0 = new MockProvider('openai')
      .setPlan([{ filePath: 'app.ts', action: 'create', reason: 'new' }])
      .setVerdicts('APPROVE'); // reviews in round 2

    const p1 = new MockProvider('qwen')
  ...
  test('orchestrate: split verdict is not consensus → continues to next round', async () => {
    // Round 1: qwen APPROVE, claude REQUEST_CHANGES → not unanimous → no consensus
    // Round 2: openai APPROVE, claude APPROVE → consensus (openai reviews when qwen builds)
    ...
    const reviewer1 = new MockProvider('qwen')
  ...
  test('orchestrate: multiple files each go through their own debate', async () => {
    ...
    const reviewer = new MockProvider('qwen').setVerdicts('APPROVE', 'APPROVE');
