...
const PREVIEW_TTL_MS = 30 * 60 * 1000; // 30-minute expiry for command preview tokens
...
export async function executePreview(token: string): Promise<{ success: boolean; message?: string }> {
  ...
  if (Date.now() - preview.timestamp > PREVIEW_TTL_MS) {
    activePreviews.delete(token);
    return { success: false, message: 'Token expired. Please regenerate the command preview.' };
  }
